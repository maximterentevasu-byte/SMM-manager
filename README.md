...
| `/story-bot` | Webhook и API управления Stories Bot |
| `/admin-bot` | Webhook @smmplatformb_bot — username→chat_id маппинг для уведомлений |
...
1. Кнопка «+ Добавить бизнес» → `POST /api/onboarding/new-business` (создаёт пустой бизнес немедленно)
2. `localStorage.businessId` = новый ID → редирект на `/onboarding`
3. Онбординг заполняет уже существующий бизнес (не создаёт новый)

### Лимит
- Backend проверяет: не более 3 бизнесов на аккаунт (HTTP 400 при превышении)
- `POST /onboarding/new-business` — создаёт пустой бизнес с проверкой лимита
...
### Канал (автопостинг)
Пользователь добавляет **@smmplatformb_bot** (`SHARED_TG_BOT_TOKEN`) администратором канала (право «Публикация сообщений»), затем вводит `@username` или числовой ID канала.  
Токен пользователя не требуется — все запросы идут через общий бот платформы.

### Уведомления администратора (admin_chat_id)
Позволяет получать в личку уведомления о постах на согласовании.

**Настройка:**
1. Написать `/start` боту [@smmplatformb_bot](https://t.me/smmplatformb_bot)
2. Бот сохранит `username → chat_id` в Redis (TTL 90 дней) и ответит ID
3. В разделе «Платформы» → «Уведомления» ввести `@username` или числовой ID

**Под капотом:** `POST /api/admin-bot/webhook` получает сообщение → пишет `tg_username:{username} = chat_id` в Redis. `PATCH /api/platforms/admin-chat/{biz}`: если вход с `@` — резолвит из Redis, иначе проверяет через `getChat`.

### .env
```
SHARED_TG_BOT_TOKEN=<токен @smmplatformb_bot>
```
