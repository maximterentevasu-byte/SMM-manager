...
export default function OnboardingPage() {
  ...
  // Шаг 1: сохранить профиль и получить уточняющие вопросы
  const saveAndClarify = async () => {
    ...
    try {
      const payload = {
        ...
        competitors: form.competitors.filter((c) => c.name || c.url).map((c) => ({
          name: c.name, url: c.url, pros: "", cons: "",
        })),
  ...
  // Шаг 2: ответить на уточнения и запустить стратегию
  const launch = async () => {
    setLaunching(true);
    setError("");
    try {
      ...
      // Запускаем контент-план на текущий месяц
      const now = new Date();
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "2rem 1rem" }}>
          ...
          {STEPS.map((s, i) => (
            <div key={s.id} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              <div style={{
                ...
                background: i <= step ? "#1a1a1a" : "#E0DED8",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div>
              ...
              <textarea value={form.usp} onChange={(e) => set("usp", e.target.value)}
                placeholder="Готовим тесто при клиенте, авторские рецепты из итальянской муки"
                style={{ ...inputStyle, minHeight: 80, resize: "vertical" }} />
              <p style={hintStyle}>Чем вы отличаетесь от конкурентов? Одним-двумя предложениями.</p>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                {PRICE_SEGMENTS.map((ps) => (
                  <div key={ps.value} onClick={() => set("price_segment", ps.value)}
                    style={{ padding: "12px", border: `1.5px solid ${form.price_segment === ps.value ? "#1a1a1a" : "#E0DED8"}`,
            ...
            <button onClick={() => setStep(1)} disabled={!form.name || !form.niche || !form.usp || !form.geo}
              style={{ padding: "13px", background: "#1a1a1a", color: "#fff", border: "none",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div>
              ...
              <input value={form.audience_primary} onChange={(e) => set("audience_primary", e.target.value)}
                placeholder="Семьи с детьми 28-45 лет, живут в спальных районах" style={inputStyle} />
            </div>

            <div>
              <label style={labelStyle}>Главные боли и проблемы клиентов</label>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setStep(0)} style={{ flex: 1, padding: "13px", background: "#F1EFE8",
                color: "#444", border: "none", borderRadius: 12, cursor: "pointer", fontSize: 15 }}>
                ← Назад
              </button>
              <button onClick={() => setStep(2)} disabled={!form.audience_primary}
                style={{ flex: 2, padding: "13px", background: "#1a1a1a", color: "#fff", border: "none",
                  borderRadius: 12, cursor: "pointer", fontSize: 15, fontWeight: 600,
        ...
        {step === 2 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div key={pl.id} style={{ border: `1.5px solid ${form.platforms.includes(pl.id) ? "#1a1a1a" : "#E0DED8"}`,
                ...
                <div onClick={() => togglePlatform(pl.id)}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", cursor: "pointer",
                    background: form.platforms.includes(pl.id) ? "#F8F7F4" : "#fff" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setStep(1)} style={{ flex: 1, padding: "13px", background: "#F1EFE8",
                color: "#444", border: "none", borderRadius: 12, cursor: "pointer", fontSize: 15 }}>
                ← Назад
              </button>
        ...
        {step === 3 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
                {BRAND_VOICES.map((bv) => (
                  <div key={bv.value} onClick={() => set("brand_voice", bv.value)}
                    style={{ padding: "12px 14px", border: `1.5px solid ${form.brand_voice === bv.value ? "#1a1a1a" : "#E0DED8"}`,
                      borderRadius: 10, cursor: "pointer", background: form.brand_voice === bv.value ? "#F8F7F4" : "#fff" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setStep(2)} style={{ flex: 1, padding: "13px", background: "#F1EFE8",
                color: "#444", border: "none", borderRadius: 12, cursor: "pointer", fontSize: 15 }}>
                ← Назад
              </button>
        ...
        {step === 4 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {clarifyQs.length === 0 ? (
              ...
              clarifyQs.map((q, i) => (
                <div key={i} style={{ background: "#fff", border: "1px solid #EAE8E2", borderRadius: 12, padding: "16px" }}>
            ...
            <button onClick={launch} disabled={launching}
              style={{ padding: "15px", background: "#1a1a1a", color: "#fff", border: "none",
