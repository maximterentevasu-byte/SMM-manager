      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #E8E6E0", padding: "0 2rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 22, fontWeight: 700, color: "#1a1a1a", letterSpacing: -0.5 }}>🍕 SMM Platform</span>
            <span style={{ fontSize: 13, color: "#888", background: "#F1EFE8", padding: "2px 10px", borderRadius: 20 }}>Май 2026</span>
          </div>
          <div style={{ display: "flex", gap: 24, fontSize: 13, color: "#666" }}>
            <span>Постов готово: <strong style={{ color: "#0F6E56" }}>{stats.ready}</strong></span>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "2rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {filtered.map((slot) => {
            ...
            return (
              <div key={slot.id} style={{ background: "#fff", borderRadius: 16, border: "1px solid #EAE8E2", overflow: "hidden" }}>
                {/* Card header */}
                <div style={{ padding: "14px 20px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #F2F0EC" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.5, color: "#888", background: "#F1EFE8", padding: "3px 8px", borderRadius: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, padding: "3px 10px", borderRadius: 20, color: st.color, background: st.bg }}>
                    {st.label}
                  </span>
                </div>
                ...
                <div style={{ display: "flex", gap: 0 }}>
                    ...
                    {isEditing ? (
                      <div>
                        <textarea value={editText} onChange={(e) => setEditText(e.target.value)}
                          style={{ width: "100%", minHeight: 200, padding: 12, fontSize: 14, lineHeight: 1.6,
                            border: "1.5px solid #533AB7", borderRadius: 10, resize: "vertical",
                            fontFamily: "inherit", outline: "none", boxSizing: "border-box" }} />
                        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                          <button onClick={() => saveEdit(slot.id)} disabled={saving}
                            style={{ padding: "8px 18px", background: "#1a1a1a", color: "#fff", border: "none",
                              borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
                            {saving ? "Сохраняю..." : "Сохранить"}
                          </button>
                          <button onClick={() => setEditingId(null)}
                            style={{ padding: "8px 18px", background: "#F1EFE8", color: "#555", border: "none",
                      ...
                      <div>
                        <p style={{ fontSize: 14, lineHeight: 1.7, color: "#2a2a2a", margin: 0, whiteSpace: "pre-wrap" }}>
                          {slot.post_text}
                        </p>
                        <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
                          <button onClick={() => startEdit(slot)}
                            style={{ padding: "7px 14px", background: "#F1EFE8", color: "#444", border: "1px solid #E0DED8",
                              borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
                            ✏️ Редактировать
                          </button>
                          {slot.status === "content_ready" && (
                            <button onClick={() => publishNow(slot)}
                              style={{ padding: "7px 14px", background: "#0F6E56", color: "#fff", border: "none",
                  ...
                  {/* Image area */}
                  <div style={{ width: 200, borderLeft: "1px solid #F2F0EC", padding: 16, display: "flex", flexDirection: "column", gap: 10, alignItems: "center", justifyContent: "center", background: "#FAFAF8" }}>
                    {slot.image_url ? (
                      <img src={slot.image_url} alt="post" style={{ width: "100%", borderRadius: 10, objectFit: "cover" }} />
                    ) : (
                      <div style={{ width: "100%", aspectRatio: "1", background: "#F1EFE8", borderRadius: 10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
                        <span style={{ fontSize: 28 }}>🖼</span>
                        <span style={{ fontSize: 11, color: "#999", textAlign: "center", lineHeight: 1.4 }}>Картинка не сгенерирована</span>
                      </div>
                    )}
                    {slot.image_prompt && !slot.image_url && (
                      <button onClick={() => generateImage(slot)} disabled={generatingImg === slot.id}
                        style={{ width: "100%", padding: "8px", background: "#533AB7", color: "#fff", border: "none",
                          borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
